...
class Import_Schema_MeanPublic extends Import_Schema {
      ...
      public function etl ($filename)
      {
            ...
            foreach ($files as $file)
            {
                  ...
                  elseif ($this->load_logs AND preg_match('/^Log\..*\.xml\.zip$/i',
                              $bname))
                  {
                  ...
                  foreach ($innerFiles as $xmlFile)
                  {
                        ...
                        $extracts = array_merge($extracts,
                              call_user_func($loader, $xml));
                        unset($xml);
                        unlink($xmlFile);
                  }
                  unlink($file);
      ...
      protected function _load_data ($xml)
      {
            ...
            if ($this->ch_filter_type === 'white' AND array_key_exists('white',
                        $this->ch_filter))
            {
                  $bool = TRUE;
                  $filter = $this->ch_filter['white'];
            }
            elseif ($this->ch_filter_type === 'black' AND array_key_exists('black',
                        $this->ch_filter))
            {
                  $bool = FALSE;
